...
Vue.extend({
  ...
  render: function render(h, _ref) {
    ...
    return h('li', [header || h(), h('ul', vueFunctionalDataMerge.mergeData(data, {
...
var makePageArray = function makePageArray(startNum, numPages) {
  return range(numPages).map(function (val, i) {
    return {
      number: startNum + i,
      classes: null
    };
...
var tbodyRowMixin = {
  ...
  methods: {
    ...
    renderTbodyRowCell: function renderTbodyRowCell(field, colIndex, item, rowIndex) {
      ...
      var $childNodes = this.normalizeSlot(["[".concat(field.key, "]"), '[]', field.key], slotScope) || toString$1(formatted);
    ...
    renderTbodyRow: function renderTbodyRow(item, rowIndex) {
      ...
      var hasRowClickHandler = this.$listeners['row-clicked'] || this.selectable; // We can return more than one TR if rowDetails enabled
